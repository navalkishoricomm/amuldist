...
app.get('/api/my/orders/:id', auth, requireDistributorOrStaff(null), async (req, res) => {
  try {
    const { id } = req.params;
    const { distributorId } = getContext(req);
    const order = await Order.findOne({ _id: id, distributorId })
      .populate({
        path: 'items.productId',
        select: 'nameEnglish nameHindi unit',
        populate: { path: 'unit', populate: { path: 'firstUnit secondUnit' } }
  ...
  try {
    ...
    for (const [pidStr, qty] of byProd.entries()) {
      ...
      if (rr) price = rr.price; else {
        const dr = await Rate.findOne({ distributorId, productId: pid }); if (dr) price = dr.price; else {
